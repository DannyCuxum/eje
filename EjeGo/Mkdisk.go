...
func AnalisiMkdisk(comando string) {
	...
	for strings.Compare(linecomand[contador], "\n") != 0 && strings.Compare(linecomand[contador], "#") != 0 {
		...
		if strings.Compare(linecomand[contador], " ") == 0 {
			...
			contador++
			//en esta parte es donde guardamos la informacion de que parametro estamos encontrando
			//-unit=
			//-size=
			//-fit=
			//-path=
	...
	//fmt.Println("Solo para q no de error: ", flag_size, flag_fit, flag_unit, flag_path)
